...
31개 클래스가 쓰이지만 **실제 스타일은 8가지**입니다(실측 대조). 새 배지 클래스를 만들지 말고 이 안에서 고릅니다.

| 톤 | 값 | 쓰임 |
|---|---|---|
| 기본 · 중립 | 투명 + `--txt` | 일반 라벨 — `.badge` `.statuspill` `.typebadge` `.agebadge` `.tbadge` `.tobadge` `.fambadge` `.waitbadge` `.rosterbadge` `.impdupbadge` `.kpi-badge` **가 모두 같은 스타일** |
| 값 있음 | 흰 배경 + 1px | 값이 채워진 상태 `.badge.has` |
| 회색 채움 | `#EEF1F5` + 회색 | 지난 · 제외 · 준비중 |
| 파랑 틴트 | `--pb-light` + `--pb-dark` | 현재 · 대상 · 입학 시기 |
| 파랑 채움 | `--pb` + 흰 | 수치 강조 |
| 진회색 채움 | `#5B6776` + 흰 | 없음 · 0 |
| 레드 틴트 | `#FDECEC` + `--danger` | 임박(D-DAY) |
| 레드 채움 | `#E6483C` + 흰 | 신규 |

공통 규격은 10.5px / 500 · 높이 16px · 라운드 999px 이고, **용도가 정해진 클래스**(`agebadge` 연령·MAP, `to-badge` 입학 시기, `ddaybadge` 잔여일)는 그 용도로만 씁니다.
